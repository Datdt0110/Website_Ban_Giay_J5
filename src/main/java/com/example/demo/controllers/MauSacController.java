...
public class MauSacController {
    ...
    public String getIndex(Model model, @RequestParam(defaultValue = "1") int page,
        ...
        if (!AuthChecker.isLoggedIn(session)) {
            ...
            if (timKiem.isEmpty()) {
                ...
                listMS = mauSacRepository.findAllPaging(pageable).getContent();
            } else {
                listMS = mauSacRepository.findByTimKiem(timKiem);
            }
            if (listMS.isEmpty()) {
    ...
    public String edit(Model model, @PathVariable("id") Integer id, HttpSession session) {
        ...
        MauSac listMSDetail = this.mauSacRepository.findById(id).get();
